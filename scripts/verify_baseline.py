...
本命 ensemble_core の3構成で確認する:
  ...
  3. 本番換算 単元BT ¥20M・8銘柄/側・信用2倍・7bps・**¥10億フロア＋規制銘柄ショート除外**
       全期間   ≈ 年率36.7% / Sh 1.74 / DD −18%
       OOS24+  ≈ 年率95.4% / Sh 3.45 / DD −18%   ※起点 2024-01-01
     （2026-07-31 schema v9 で shortable を PIT 化。旧値 79.1%/2.98 は貸借フラグが
      2026年スナップショット由来だった頃の値で、**過小評価**だった）
     （旧「101%/Sh3.21」は¥5億フロア時代の値。条件が違うので比較に使わない。
      期待値の正本は下の EXPECTED であって、この docstring ではない）
...
EXPECTED = [
    ...
    ("リサーチ @3bps OOS24-08+", {"sharpe": 5.33}),
    ("リサーチ @7bps OOS24-08+", {"sharpe": 3.43}),
    # 2026-07-30 更新: 規制銘柄ショート除外（発注拒否前提）+ 全面流動性フロア¥10億が本番条件
    # 2026-07-31 更新: shortable の PIT 化（schema v9）。master.parquet は2026-07-27の
    # 1枚スナップショットで、そこから作った貸借フラグを日付キーなしで全期間に当てていた
    # ＝ルックアヘッド。週次 margin_interest の IssType で当時の区分に復元した。
    # **除去すると成績は上がった**（OOS24+ Sh3.04→3.45）ので、旧値は過小評価だった。
    ("単元¥20M 8/側 2.0x 7bps 全期間", {"sharpe": 1.74, "ann_return": 0.367, "max_drawdown": -0.183}),
    ("単元¥20M 8/側 2.0x 7bps OOS24+", {"sharpe": 3.45, "ann_return": 0.954, "max_drawdown": -0.183}),
]
