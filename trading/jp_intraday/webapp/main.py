...
<div class=warn><b>⛔ 決着（2026-07-31）: 実弾 NO-GO — 本戦略は現構成では執行不能</b><br>
実測により<b>「気配値と寄り付き値に有効な関係なし・予測も収束もしない」</b>が確定。
事前登録した基準（建玉一致率&lt;50%→NO-GO）に照らし実弾NO-GO。論理の鎖:
①αは当日寄付きギャップにしか無い（外すとgross年率 最良+4.9% vs 本戦略+69.4%）→
②そのαは板寄せクロス内でしか受け取れない（60秒で69%消失）→
③遅延エントリーは代替にならない（コスト0でも理論上限Sh1.59 vs 板寄せ3.29）→
④クロス参加には確定前の発注＝当日ギャップの事前推定が必須 →
⑤その気配が寄値を予測しない。<b>αは実在するが到達不能</b>。
データ源を変えても直らない（特別気配は規制上の段階値であり、かつ
「気配が当たる銘柄」と「αがある銘柄」がほぼ排他＝|ギャップ|最上位五分位は
09:00約定率4.1%なのにαの54.4%を生む）。<br>
<b>以下の成績はシミュレーションとしては引き続き有効</b>だが、
シグナルを実時間で入手する経路が無いため実運用はできない。</div>

<div class=warn><b>（参考・経緯）特別気配のメカニズム</b><br>
...
上限クリップでもSharpeは下げ止まる。<b>致命的なのはランダム誤差のみ</b>。<br>
<b>実弾GO/NO-GOの基準は2026-07-31に事前登録済み</b>（実測を見る前に確定させ、後出しで
緩める余地を潰した）。既知の歪みを本番と同じ経路に注入して作った対応表:
<table style="margin:6px 0"><tr><th>歪み</th><th>建玉一致率</th><th>Sharpe</th><th>維持率</th></tr>
<tr><td>なし（基準）</td><td>100%</td><td>3.04</td><td>100%</td></tr>
<tr><td>一様圧縮 λ=0.5 / 0.3</td><td>100%</td><td>3.04</td><td><b>100%</b></td></tr>
<tr><td>±3%クリップ</td><td>78.6%</td><td>2.45</td><td>81%</td></tr>
<tr><td>ランダム σ=100bps</td><td>53.7%</td><td>2.21</td><td>73%</td></tr>
<tr><td>ランダム σ=250bps</td><td>36.6%</td><td>1.40</td><td>46%</td></tr>
<tr><td>ランダム σ=500bps</td><td>30.2%</td><td>0.83</td><td>27%</td></tr></table>
<b>建玉一致率 ≥75%→実弾GO / 50-75%→¥5Mに減額して開始 / &lt;50%→NO-GO</b>。
一様圧縮は成績が基準と完全一致＝<b>λがいくら小さくても無害</b>なのでλ単独では判断しない。
順位相関も使わない（クリップは順位相関1.000でも一致率78.6%）。<br>
