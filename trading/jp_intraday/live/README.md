...
| 07:25 login | 07:40 collect | 08:20 plan | 08:40 probe | 08:52 entry | 14:55 exit | 15:40 state |
|---|---|---|---|---|---|---|

- **plan と entry を重ねてはいけない**（2026-07-30 に 08:44 plan が 08:52 entry に食い込んで発覚）。
  どちらも `open_prices()` でユニバース全銘柄の板を引き、実測 **~6件/秒で5〜7分**かかる。
  同時に走ると各プロセス ~9.5req/s × 2 で参照系の **10req/s 制限を超えて 429**、板が欠けると
  entry の「カバレッジ<80%で中止」ガードに掛かり**その日の発注が丸ごと飛ぶ**。
  → plan は 08:20（entry の30分前・完了まで見ても余裕）に前倒し済み。
  plan は情報表示専用（発注しない）ので、entry と競合したら plan を落として構わない。
