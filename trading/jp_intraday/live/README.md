...
- **ユニバース: 全面流動性フロア¥10億**（`LIVE_MIN_VALUE_YEN=1e9`・2026-07-30ユーザー決定。
  626銘柄/日・**板取得は約14分**（下記「板取得は一周20分かかる」の実測。~66秒はレート制限からの
  理論値で、実際は1銘柄~900ms）。測定済みコスト: OOS24+ 年率101%→79%・Sh3.25→2.98=執行品質との交換。
...
3. `run_live entry` … 板取得 →
   ...
   **⚠️開始時刻は板取得の実所要で決まる。ここは数字が食い違っている（要確認）**:
   - 「~66秒」は **スロットル定数からの理論値**（MIN_INTERVAL=0.105s → 9.52件/秒 ＝ 467/9.5=49秒）
   - **Windows本番での実測は 0.7〜1.3件/秒**（＝理論の1/10）。証拠:
     2026-07-30 entry の板取得 08:58:48→09:17:50 で **863銘柄/19分・カバレッジ100%**、
     07-31 quotesnap も 40銘柄で50〜89秒。**626銘柄なら約14分**
   - この差のため 08:48 開始では寄付きに間に合わず、**2026-07-29/30 に2日連続で entry 不発**
     （09:17発注の寄成は板寄せに乗らず State5 で全消滅）
   - 実測が正なら「66秒スメア」の前提も1桁変わる（→ §0 の判定に影響）。
     **どちらが正しいかを朝の実測ログで確定させること**
...
python -m trading.jp_intraday.live.run_live entry     # 08:48 寄成 新規建て（冪等・板取得~66秒）
