...
1. タスクスケジューラに平日 **08:47** 起動で1本追加（`TradeBot-6-QuoteSnap` として登録済み）:
   `PYTHONUTF8=1 python -m trading.jp_intraday.live.run_live quotesnap`
   板寄せ気配を **08:50/08:55/08:59 の3時点**で記録。発注なし・read-only。
   **⚠️1時点あたりの銘柄数は `.env` の `QUOTESNAP_LIMIT`（既定60）で絞る**。
   板は実測~1秒/銘柄なので全626銘柄では1時点=約11分かかり、3時点が互いに重なって
   「その時刻の気配」でなくなる（08:55/08:59分が寄付き後にずれ込む）。
   等間隔サンプリングなので流動性レンジ全体を拾い、時点間で同一銘柄になる。
   σの推定には60銘柄×3時点×日数で足りる。テール銘柄を厚く見たくなったら層化に変える。
2. 夕方のデータ収集後に `PYTHONPATH=. python scripts/analyze_quotesnap.py`
   → 時点別の乖離分布と「|気配ギャップ|≥3%テール銘柄の系統的縮小率」が出る
3. **判定**: ★旧基準「σ≤25bps→影響ゼロ / σ≈51bps→Sh−10% / σ≥100bps→要再設計」は
   **撤回済み**。誤差をiidランダムと仮定した机上値で、実際の気配誤差は特別気配の離散更新
   （¥2,000-3,000帯で1step=¥50=205bps）に支配され、機構的下限だけでα加重1,060bps
   ＝旧基準は桁が2つ足りない。**「乖離の大きさ」では判定できない**。
   代わりに `analyze_quotesnap.py` が誤差を分解して出力する（**合否は判定しない**）:
   - **λ（圧縮率）** → それ自体は無害。決定論的なら補正で100%復元
   - **R²（★判別子）** → 高い=系統的で補正可能 / 低い=真のランダム成分。**λでなくR²で分ける**
   - **σ_ε** → R²が低いときだけ意味を持つ（一律σ250bpsでSharpe半減・310bpsで1.0）
   - **選択一致率**（気配で選ぶ上位/下位8本 vs 実寄値で選ぶそれ）→ **戦略に直結する唯一の量**
   - ✗**順位相関は判定に使わない**（±3%クリップは順位相関1.000なのに選択一致12%）
   計測器自体の妥当性は `python scripts/analyze_quotesnap.py --selftest` で確認できる
   （既知の歪み5種を注入してλ・σの回収精度を表示。2026-07-31に回帰の向きのバグを修正済み）
   **合格基準は2026-07-31に事前登録済み**（実測を見る前に確定＝後出しで緩める余地を潰した）。
   `python scripts/analyze_quotesnap.py --book` が出す**建玉一致率**を対応表に当てる:
   **≥75%→実弾GO / 50-75%→¥5Mに減額して開始 / <50%→NO-GO**
   （対応表: 100%=Sh3.04 / 78.6%=2.45 / 53.7%=2.21 / 36.6%=1.40 / 30.2%=0.83。
   作成は `scripts/quote_distortion_calibration.py`・詳細は AGENTS.md）。
   **一様圧縮λは一致率100%＝完全に無害**なのでλ単独では判断しない。
...
- **ユニバース: 全面流動性フロア¥10億**（`LIVE_MIN_VALUE_YEN=1e9`・2026-07-30ユーザー決定。
  626銘柄/日・**板取得は実測20〜35分**（未登録銘柄の /board が中央値~900ms のため。
  「~66秒」はスロットル定数からの理論値で実測ではない）。
  測定済みコスト: OOS24+ 年率101%→79%・Sh3.25→2.98=執行品質との交換。
...
- **コスト感応度（OOS24+・現行本番条件で再計算 2026-07-31）**:

  | コスト/side | 年率 | Sharpe | maxDD | 日次勝率 |
  |---|---|---|---|---|
  | 2bps（板寄せ実測ベース） | 127% | 4.59 | −15.1% | 59.9% |
  | 5bps | 101% | 3.66 | −17.0% | 57.2% |
  | **7bps（現行の正本・保守）** | **84%** | **3.04** | **−18.7%** | **55.3%** |
  | 10bps | 58% | 2.10 | −21.3% | 52.6% |
  | 14bps（旧前提） | 24% | 0.86 | −31.3% | 48.7% |

  データ窓は日次2018-01〜2026-07-28。`verify_baseline.py` の期待値（7bps: 79.1%/Sh2.98）は
  **2026-07-24スナップショット時点で固定した環境再現アンカー**なので、データ延長ぶん
  （+数営業日）だけ上の実測とズレる（許容±10%内）。**アンカーは動かさない**。
  （旧記載「5bps=118%/Sh3.8、7bps=101%/Sh3.2」は¥5億フロア時代の値なので破棄）。
...
> 実取引可能性は4方向で検証済み: ①執行パイプライン（preflight/plan実出力・全テストgreen）
...
3. `run_live entry` … 板取得 →
   寄成発注（**0.125秒間隔＝8件/秒**・自動リトライ・冪等）。
   ※発注レートは2026-07-10のVer5.44.0.0で5→10件/秒に緩和済み。
   **⚠️開始時刻は板取得の実所要で決まる（2026-08-03 実測で決着）**:
   - 旧記載の「~66秒」は **スロットル定数からの理論値**（MIN_INTERVAL=0.105s → 9.52件/秒）で
     **実測ではなかった**。未登録銘柄の /board は中央値~900ms かかる
   - **Windows本番の実測 0.32〜1.03件/秒**:
     2026-07-30 entry 863銘柄19分（08:58:48→09:17:50・カバレッジ100%）/
     2026-08-03 寄前 `sweep 450/686 経過1418秒`＝**0.32件/秒・686銘柄で約35分**/
     2026-08-03 場中 140銘柄135秒＝1.03件/秒
   - この差のため 08:48 開始では寄付きに間に合わず、**2026-07-29/30 に2日連続で entry 不発**
     （09:17発注の寄成は板寄せに乗らず State5 で全消滅）
   - **スメアは1分ではなく20〜35分**。§0 の判定はこの前提で読み直すこと
   **確定間際の気配は使わない（使えない）前提の運用**: バックテストは確定寄値でシグナルを
   計算しているため、08:50気配とのズレは選択ノイズになる。
   ★旧「σ25bpsまで劣化ゼロ・51bpsで−10%」は**撤回**（iidノイズ仮定で桁が2つ足りない）。
   実際は特別気配の離散更新に支配され機構的下限でα加重1,060bps。
   ただし**歪みが系統的なら床Sharpe1.8で生存**（一様圧縮は補正可能・クリップでも1.85）、
   **ランダム250bps級なら半減**。判定は quotesnap の系統/ランダム分解で行う
...
**prod移行条件（全て揃ってから）**: (a) **参加率p90が寄り≤3%/引け≤1.5%・数量約定率≥98%**
（※「実効コスト5日平均≤Xbps」は板寄せでは構造的に測れないので条件から削除）
(b) 寄成/引成の全量約定を確認 (c) CalcPrice・HoldID返済・MarginSell/KCMarginSellの実接続確認
(d) **quotesnapの系統/ランダム分解で気配前提が成立すること**。

### 板寄せ執行の監視（**2026-07-30 全面改訂**・初日から記録）

**測れないものと測れるものの区別（重要）**: 寄成・引成は板寄せ＝**単一約定価格**なので、
約定できた限り「実約定価格 = 公式寄値/引値」で**乖離は構造的にゼロ**。自分のインパクトは
公式価格そのものに内包されており、バックテストも同じ価格を使うため**実現P&Lとの突合からは
原理的に分離できない**。したがって監視するのは「コストそのもの」ではなく
**コスト前提が崩れる兆候**である。

**オフライン推定の結論（実測3系統が収束）**: 板寄せの真のコストは
**寄り約1.5bps/side・引け約0.5bps/side（往復2.0bps・レンジ0.6-4.5）**。
根拠: ①参加率実測（¥20M本番構成で寄り¥加重1.0-1.3%・引け0.44%。**引けの板は寄りの3-5倍厚い**）
②板寄せの用量反応（平常の6倍の一方向フローで+26bps→我々の¥1.12Mに外挿して0.2-0.4bps）
③ザラ場Kyleλ（0.23bps・上界）④引け15:24→15:30の実測スリップ（+0.52±0.70bps・上界）。
**現行前提の7bps/sideは3.5〜7倍の過大**（OOS24+で Sharpe 3.04→4.59・年率+43pt・+¥8.6M/年の差）。
※R9の「呼値ハーフスプレッド1.21bps/side」は**ザラ場指値の話であり板寄せには足さない**。

**毎日の記録**（Windowsタスクに1本追加・15:40以降）:
```
PYTHONUTF8=1 python -m trading.jp_intraday.live.run_live cost
```
**集計と判定**: `PYTHONPATH=. python scripts/analyze_effective_cost.py`

| 監視指標 | 閾値 | 意味 |
|---|---|---|
| **参加率 p90** | 寄り ≤3% / 引け ≤1.5% | インパクトの唯一の観測可能なdriver。超えたらコスト前提の再検討 |
| **数量約定率** | ≥98% | 寄らず・部分約定の検知 |
| **\|slip\|（fill vs 公式価格）** | ≤0.5bps | **0が正常**。非ゼロは板寄せ外約定（SORのPTS迂回等）の異常検知 |
| **プレミアム料** | 発生有無を別建て | 一日信用は上限100bps/日で**桁が違う**。ここだけは実費として計上 |

**コスト前提の扱い**: `verify_baseline.py` の7bps期待値は**環境再現アンカーとして据え置く**
（成績の主張ではない）。正本の差し替えは**ライブ実測40営業日の合格後**に行う。
低コストを根拠に既存の棄却候補を再選抜するのは**選択行為**なので、
新しい未使用期間で判定式を事前固定して1回だけ実施すること。
...
python -m trading.jp_intraday.live.run_live quotesnap # 08:47 気配3時点の記録（発注なし・read-only）
python -m trading.jp_intraday.live.run_live entry     # 寄成 新規建て（冪等・板取得は実測20〜35分）
python -m trading.jp_intraday.live.run_live exit      # 14:55 引成 返済（全フラット・再実行安全）
python -m trading.jp_intraday.live.run_live state     # 15:30 建玉/資産を管理画面へ送信
python -m trading.jp_intraday.live.run_live cost      # 15:40 参加率/約定率の記録
python -m trading.jp_intraday.live.run_live shadow    # 09:01〜15:32 仮想TP/SL監視（発注なし・下記）
python -m trading.jp_intraday.live.run_live plan      # 任意: 発注せず内容だけ確認（手動用）
```
...
- [x] 毎営業日 08:45 に発注経路プローブ（約定不能指値→即取消、`scripts/preflight_order_probe.py`）。
      entry(08:48) 前に口座抑止・認証切れをダッシュボードで検知できる
- [ ] 寄成の想定始値 vs 実約定のズレ（板の予想始値精度）— 実弾初日以降に計測
- [ ] 少額（¥5M・5銘柄/側から）で本番を数日 → 段階増資

## Windows VPS 運用（タスクスケジューラ登録済み・平日JST）
| 07:25 login | 07:40 collect | 08:47 quotesnap | 14:55 exit | 15:40 state | 15:45 cost | 19:30 quotesnap分析 |
|---|---|---|---|---|---|---|

**現在 entry / probe / plan は無効**（§0 の実弾NO-GO に従う。発注する経路は止めてある。
`.env` も `KABU_DRY_RUN=1`）。再開時の起動時刻は**板取得の実所要で決まる**:
「08:48起動（板取得~66秒）」はスロットル定数からの理論値で、**Windows実測は0.7〜1.3件/秒
＝626銘柄で約14分**（証拠は上記の実測表）。実測が正なら 08:48 では寄付きに間に合わない。
`plan` は定期実行しない（手動確認用）。
