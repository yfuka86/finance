...
3. **判定**: ★旧基準「σ≤25bps→影響ゼロ / σ≈51bps→Sh−10% / σ≥100bps→要再設計」は
   **撤回済み**。誤差をiidランダムと仮定した机上値で、実際の気配誤差は特別気配の離散更新
   ...
   - **系統成分（圧縮率λ = 原点通過回帰の傾き）** → λが安定なら補正可能・情報は残る
     （λ=0.3の一様圧縮でも順位相関1.000・床Sharpe1.8で生存）
   - **ランダム成分（残差σ）** → ここだけが致命的（全銘柄一律σ250bpsでSharpe半減・310bpsで1.0）
   - **大ギャップ帯のλ**（クリップ検知）→ クリップでも1.85だがsvdnスリーブは死ぬ
