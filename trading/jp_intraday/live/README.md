...
### 日次の実効コスト実測（**脚別に分離**・初日から必ず記録）

**2026-07-30 更新**: 往復合算でなく**寄成(entry)と引成(exit)を分離**して記録する。
理由（実測）: 引け板寄せ15:30は日次売買代金の**13.5%**、寄付板寄せ9:00は**4.0%**で
**出口の板は入口の3.4倍厚い**。同じ建玉でも参加率は寄り側が約4倍なので実効コストは
構造的に非対称で、往復平均で持つと入口の悪さが出口に隠れる。

**毎日の記録**（Windowsタスクに1本追加・15:40以降）:
```
PYTHONUTF8=1 python -m trading.jp_intraday.live.run_live cost
```
→ `data/live_reports/cost_YYYY-MM-DD.jsonl` に約定ごとの
「実約定価格 vs 公式寄値/引値」の乖離(bps)を記録し、サマリをWeb管理画面へ送信。
板寄せは単一約定価格なのでスプレッドは原理的にゼロ＝**この乖離がそのまま
自分の注文が板寄せ価格を動かした分（マーケットインパクト）の実測値**になる。

**集計と判定**（数日ごと。J-Quantsの確定した公式寄値/引値で自動的に上書き再計算される）:
```
PYTHONPATH=. python scripts/analyze_effective_cost.py
```
- 5日移動平均の**往復** ≤14bps（=片道7bps相当）: 継続
- 14〜20bps: 継続するが**増額凍結**
- >20bpsが5日継続: **サイズ半減**（¥10M）
- >26bps: **停止**して原因分析（全期間の損益分岐≈28bps往復）

**期待される上振れ**: 本番前提の7bps/sideは保守的な可能性が高い（呼値ベースの実測は
流動銘柄で1.21bps/side・Roll推定1.39bps/side）。実勢が片道3〜5bpsなら
**年率で最大+17pt≒+¥340万/年**の上振れになる。ただし板寄せのインパクトは
呼値スプレッドとは別物なので、**この実測が出るまでコスト前提は7bpsのまま据え置く**。
あわせて記録: 寄成の不成立/部分約定・ショートのプレミアム料/在庫切れ（銘柄別）。
