...
3. **08:20** `run_live entry` … 板取得(**実測~14分**/626銘柄・進捗はログに45件ごと)
   → 寄成発注（自動リトライ・冪等、〜08:40頃完了）。
   **08:48開始では間に合わない**（一周14分＝09:02完了。2026-07-29/30 に2日連続で
   entry 不発になった原因がこれ。09:17発注の寄成は板寄せに乗らず State5 で全消滅）。
   気配の鮮度と寄付き参加のトレードオフであり、**参加できないと成績はゼロ**なので前倒しを優先する。
