...
| 07:25 login | 07:40 collect | 08:10 probe | **08:20 entry** | 14:55 exit | 15:40 state |
|---|---|---|---|---|---|

（plan タスクは**無効化**。理由は下記の時間予算。`Enable-ScheduledTask TradeBot-2-Plan` で戻せる）

### ⏱ 板取得は一周20分かかる（2026-07-30 実測・スケジュールの律速条件）

`open_prices()` はユニバース全銘柄（現在863）の板を1件ずつ引く。ここが運用時間を決める。

| 条件 | 一周 | カバレッジ |
|---|---|---|
| 直列・寄付き前後（08:58開始の実績） | **19分** | 100% |
| 直列・場中（13:30開始の実測） | 26分 | 99.7% |
| 並列8+一括登録（**却下**） | 16分 | **63%** ＝有効0.56件/秒で直列より遅い |

- 未登録銘柄の `GET /board` は中央値 **~900ms**（1割はちょうど5秒）。登録済みなら3ms。
  遅延はステーションが取引所へ取りに行く部分で、**クライアント側の工夫では抜けない**
  （並列化・一括登録・登録後の待機・再取得によるウォーム化を全て実測して否定）。
  90銘柄のベンチでは並列が1.45件/秒・成功率100%に見えるが、**全ユニバースでは再現しない**
  （`unregister/all` がタイムアウト→登録が残り上限50に達してチャンクごと失敗）。
  並列版は `KabuClient.boards()` として残置（既定OFF・`KABU_BULK_BOARDS=1` で有効）。
- **時間予算**: 気配が立つ 08:00 〜 シグナル締切 08:50 の**50分に一周(20分)が1回だけ**。
  → **entry を 08:20 開始**（08:40〜08:46 取得完了 → 規制チェック → 08:48頃 発注）。
  → **plan は無効化**（同じ一周をもう一度やる余裕がない。情報表示専用なので実害なし）。
- **plan と entry を重ねてはいけない**（2026-07-30 に 08:44 plan が 08:52 entry に食い込んで発覚）。
  同時に走ると参照系 10req/s を食い合って板が欠け、entry の「カバレッジ<80%で中止」ガードで
  **その日の発注が丸ごと飛ぶ**。
- 進捗は45件ごとにログへ出る（`board 450/863 (52%) 経過863秒 残り~792秒`）。
  出ていなければ止まっている。ランナーは `PYTHONUNBUFFERED=1` を強制（無いと進捗が見えない）。
