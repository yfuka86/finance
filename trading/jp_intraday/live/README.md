...
3. **08:48** `run_live entry` … 板取得(~66秒・**シグナル気配は08:50までのスナップショット**)
   → 寄成発注（**0.125秒間隔＝8件/秒**・自動リトライ・冪等、〜08:51頃完了）。
...
| 07:40 collect | 08:45 probe | 08:47 quotesnap | 08:48 entry | 09:01 shadow | 14:55 exit | 15:30 state | 15:40 cost |
|---|---|---|---|---|---|---|---|

**entry は 08:48 起動**（¥10億フロアで626銘柄・板取得に~66秒かかるため、シグナル用の気配
スナップショットが 08:50 までに揃うよう逆算した時刻）。probe は entry の板取得と競合しないよう
**08:45**（旧 08:50 は entry 起動後になるので前倒し）。`plan` は定期実行しない（手動確認用）。
