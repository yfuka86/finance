...
**成績の主張はOOSのみ（IS成績の使用は全面禁止・AGENTS.md「OOS絶対規律」参照）**

| 指標 | 値（すべてOOS） |
|---|---|
| 本命戦略 | `ensemble_core`（ML予測強度ウェイト × セクターボラ集中版、資本50/50） |
| リサーチOOS (2024-08+) | ネットSharpe **5.09 @3bps / 3.13 @7bps**（2026-07-30 借株可能性フィルタ修正後。修正前の8.77/7.14は実行不能ショート込みの過大評価） |
| 本番換算 OOS24+（¥20M・単元・信用2倍・7bps・流動性≥¥10億・規制銘柄除外） | **年率79% / Sh 2.98 / DD−19%** |
